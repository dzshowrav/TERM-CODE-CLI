...
type ProviderEditScreen struct {
	width      int
	height     int
	id         string
	name       string
	...
	result     string
	testStatus string
	onSubmit   func(id, name, baseURL, apiKey, desc string) string
	onTest     func(id, name, baseURL, apiKey, desc string) string
}

func NewProviderEditScreen(id, name, baseURL, apiKey, desc string,
	onSubmit func(id, name, baseURL, apiKey, desc string) string,
	onTest func(id, name, baseURL, apiKey, desc string) string,
) *ProviderEditScreen {
	return &ProviderEditScreen{
		width:      80,
		height:     24,
		id:         id,
		name:       name,
...
func (s *ProviderEditScreen) SetSize(w, h int) {
	s.width = w
	s.height = h
}

func (s *ProviderEditScreen) Update(msg tea.Msg) (DialogScreen, tea.Cmd) {
	switch msg := msg.(type) {
	case tea.PasteMsg:
		paste := msg.String()
		switch s.focusField {
	...
	case tea.KeyMsg:
		...
		case "enter":
			if s.focusField < 4 {
				if s.onSubmit != nil {
					s.result = s.onSubmit(s.id, s.name, s.baseURL, s.apiKey, s.desc)
				}
				s.done = true
			} else if s.focusField == 4 {
				if s.onTest != nil {
					s.testStatus = s.onTest(s.id, s.name, s.baseURL, s.apiKey, s.desc)
				}
...
func (s *ProviderEditScreen) addChar(ch string) {
	switch s.focusField {
	case 0:
		s.name += ch
	case 1:
...
func (s *ProviderEditScreen) deleteChar() {
	switch s.focusField {
	case 0:
		if len(s.name) > 0 {
...
func (s *ProviderEditScreen) Done() bool     { return s.done }
func (s *ProviderEditScreen) Result() string { return s.result }

func (s *ProviderEditScreen) View() string {
	header := styles.H1.Render("Edit Provider: " + s.origName)
	...
	saveBtn := formBtnNormal.Render("[ Save Changes ]")
	if s.focusField == 4 {
		testBtn = formBtnActive.Render("[ Test Connection ]")
	}
	if s.focusField == 5 {
		saveBtn = formBtnActive.Render("[ Save Changes ]")
	}
	...
	if s.testStatus != "" {
		...
		if strings.HasPrefix(s.testStatus, "Error") || strings.HasPrefix(s.testStatus, "error") || strings.HasPrefix(s.testStatus, "Failed") {
